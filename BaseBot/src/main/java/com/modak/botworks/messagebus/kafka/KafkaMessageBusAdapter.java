...
public class KafkaMessageBusAdapter extends BasicMessageBusAdapter {
    ...
    public void register(MessageReceiver messageReceiver) throws MessageBusException {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (pull == false && shutdown == false) {
                                ...
                                try {
                                    ...
                                    logger.debug(
                                        ...
                                        decryptedMessageBtArray != null ? MiscUtils
                                            .formatString(new String(decryptedMessageBtArray, StandardCharsets.UTF_8),
                                                true) : null);
...
//                                    }
    ...
    public String fetchMessage() throws MessageBusException {
        ...
        while (pull == true && shutdown == false) {
            //If isPause is set to true then Polling stops untill isPause is set to false
            if (isPause == true) {
                try {
                    Thread.sleep(3000);
                }
                ...
                int checkPriorityOrder = 1;
                for (KafkaConsumer kafkaConsumer : kafkaConsumers) {
                    ConsumerRecords<String, byte[]> records = kafkaConsumer.poll(100);
                    logger
                        .trace("Number of records received : {}, Bot : {}, Bot UUID: {}", records.count(), botClassName,
                            botUUID);
                    for (ConsumerRecord<String, byte[]> record : records) {
                        //System.out.println("Record is being processed" + record.value());
                        ...
                        try {
                            byte[] bytes = BotsMessageEncryptionUtils
                                .decryptAndUncompressByteArray(kafkaMessage, privateKey);
                            String decryptedMessage = bytes != null ? new String(bytes, StandardCharsets.UTF_8) : null;
                            logger.debug(
                                "Message received topic = {}, offset = {}, partition = {},\n decryptedMessage = {} ",
                                record.topic(), record.offset(), record.partition(),
